...
def c7s1_k(input, k, reuse=False, norm='instance', activation='relu', is_training=True, name='c7s1_k'):
  ...
  with tf.variable_scope(name, reuse=reuse):
    weights = _weights("weights",
      shape=[7, 7, input.get_shape().as_list()[3], k])  # (7, 7, 3, 64)

    padded = tf.pad(input, [[0,0],[3,3],[3,3],[0,0]], 'REFLECT')  #(1, 262, 262, 3)
    conv = tf.nn.conv2d(padded, weights,
        strides=[1, 1, 1, 1], padding='VALID')

    normalized = _norm(conv, is_training, norm)  # (1, 256, 256, 64) batch normornize
...
def dk(input, k, reuse=False, norm='instance', is_training=True, name=None):
  ...
  with tf.variable_scope(name, reuse=reuse):
    weights = _weights("weights",
      shape=[3, 3, input.get_shape().as_list()[3], k])  # (3, 3, 64, 128)

    conv = tf.nn.conv2d(input, weights,
        strides=[1, 2, 2, 1], padding='SAME')
    normalized = _norm(conv, is_training, norm)
    output = tf.nn.relu(normalized)  # (1, 128, 128, 128)
    return output
  ...
  with tf.variable_scope(name, reuse=reuse):
    with tf.variable_scope('layer1', reuse=reuse):
      weights1 = _weights("weights1",
        shape=[3, 3, input.get_shape()[3], k]) #(3,3,3,k)
...
def uk(input, k, reuse=False, norm='instance', is_training=True, name=None, output_size=None):
  ...
  with tf.variable_scope(name, reuse=reuse):
    ...
    fsconv = tf.nn.conv2d_transpose(input, weights,
        ...
        strides=[1, 2, 2, 1], padding='SAME')
#    fsconv=tf.layers.conv2d_transpose(input,output_size,3,strides=2,padding='same')
...
def last_conv(input, reuse=False, use_sigmoid=False, name=None):
  ...
  with tf.variable_scope(name, reuse=reuse):
    weights = _weights("weights",
      shape=[4, 4, input.get_shape()[3], 1])  #weights.shape=(4,4,512,1)
    biases = _biases("biases", [1])#input.shape=(1,16,16,512)

    conv = tf.nn.conv2d(input, weights,
        strides=[1, 1, 1, 1], padding='SAME')
    output = conv + biases   #output.shape=(1,16,16,1)
    if use_sigmoid:
      output = tf.sigmoid(output)
    return output

### Helpers
def _weights(name, shape=None, mean=0.0, stddev=0.02):
...
def _norm(input, is_training, norm='instance'):
  ...
  """
  if norm == 'instance':
    return _instance_norm(input)  #checked
  elif norm == 'batch':
...
def _instance_norm(input):
  ...
  with tf.variable_scope("instance_norm"):
    depth = input.get_shape()[3]   #dimension(64)      input.shape=(1, 256, 256, 64)
    scale = _weights("scale", [depth], mean=1.0)   #shape=(64,)
    offset = _biases("offset", [depth])   #shape=(64,)
    mean, variance = tf.nn.moments(input, axes=[1,2], keep_dims=True)  # (1, 1, 1, 64) nomornize in axis=0&1
    epsilon = 1e-5
